...
% treatment effects on activity in resting conditions
...
fprintf('\nOT-specific effect on RS activity in %d features\n',...
    sum(OT_spec_effects));
...
fprintf('\nPL-specific effect on RS activity in %d features\n',...
    sum(PL_spec_effects));
for ftr = 1:length(log.ftr_names)
...
fprintf('\nNonspecific effect on RS activity in %d features\n',...
    sum(nonspec_effects));
for ftr = 1:length(log.ftr_names)
