...
% 1. pre,oxt/plc,rs
% 2. pre,oxt/plc,eye
% _  bcsr implemented in other script. 
...
% apply 2 tests, get pvals & pFDR from each
...
test.idx = [(idx.PRE & idx.RS & idx.oxytocin),...
            (idx.PRE & idx.RS & idx.placebo)];
...
test.idx = [(idx.PRE & idx.EYE & idx.oxytocin),...
            (idx.PRE & idx.EYE & idx.placebo)];
...
input.tableCaption = 'Significantly asimilar features';
input.tableLabel = 'presim_ftr';
...
txt = {'pre-treatment','oxytocin','placebo'};

if FDR_all
    figure
    title('Similarity tests (presim)')
    for i=1:2
        subplot(2,1,i)
        subplot_ranked_pvals(tbl.pval{i},tbl.qval{i},test.alpha,ftr_ref.ID_str,txt{i},'all');
    end
    subplot(2,1,1)
    legend('p-values','q-values','alpha')

    figure
    title('Overview of presim tests')
    for i=1:2
        subplot(2,1,i)
